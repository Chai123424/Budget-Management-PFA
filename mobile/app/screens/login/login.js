    ...
    try {
      ...
      if (action === "Login") {
        ...
        try {
            ...
            if (parsedTempData.formData) {
              try {
                ...
                if (updateResponse.ok) {
                  console.log('Données budgétaires synchronisées');
                }
              } catch (syncError) {
                console.log('Erreur sync budget:', syncError);
              }

              await AsyncStorage.setItem('budgetFormData', JSON.stringify(parsedTempData.formData));
            }

            // Restaurer les dépenses manuelles
            if (parsedTempData.expenses && parsedTempData.expenses.length > 0) {
              try {
                const expensesResponse = await fetch(`${API_URL}/users/save_expenses`, {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${response.data.token}`,
                  },
                  body: JSON.stringify({ expenses: parsedTempData.expenses })
                });

                if (expensesResponse.ok) {
                  console.log('Dépenses manuelles synchronisées');
                }
              } catch (syncError) {
                console.log('Erreur sync dépenses:', syncError);
              }

              await AsyncStorage.setItem('expenses', JSON.stringify(parsedTempData.expenses));
            }
          ...
          console.error('Erreur restauration données temporaires:', error);
        }
        ...
        try {
          ...
          const expensesResponse = await fetch(`${API_URL}/users/get_expenses`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${response.data.token}`,
          ...
          if (expensesResponse.ok) {
            ...
            if (!existingExpenses && expensesData.expenses) {
              await AsyncStorage.setItem('expenses', JSON.stringify(expensesData.expenses));
            }
          }
